...
template <typename T>
__global__
void device_saxpy(const size_t n, const T a, const T* x, T* y, T* z) {
...
int main() {
    ...
    CUDA_CHECK(cudaMallocHost(&z_h, n * sizeof(DTYPE)));


    DTYPE *x_d, *y_d, *z_d;
    CUDA_CHECK(cudaMalloc(&x_d, n * sizeof(DTYPE)));
    CUDA_CHECK(cudaMalloc(&y_d, n * sizeof(DTYPE)));
    CUDA_CHECK(cudaMalloc(&z_d, n * sizeof(DTYPE)));
    ...
    CUDA_CHECK(cudaMemcpy(z_d, z_h, n * sizeof(DTYPE), cudaMemcpyHostToDevice));
    ...
    double gpu_time = 0.0;
    for (int iter = 0; iter < runs; iter++) {
        device_saxpy<<<blocks, threads>>>(n, a, x_d, y_d, z_d);
        timer.start();
        device_saxpy<<<blocks, threads>>>(n, a, x_d, y_d, z_d);
